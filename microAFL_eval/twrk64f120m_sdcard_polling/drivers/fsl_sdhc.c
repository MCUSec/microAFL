...
// ETMFuzz
extern unsigned int etm_tc_len __attribute__((section(".non_init")));
extern unsigned char etm_tc[10000] __attribute__((section(".non_init")));
extern unsigned int etm_tc_idx;
extern int Delay_SD_ApplicationSendOperationCondition;
// ETMFuzz end
/*******************************************************************************
 * Prototypes
 ******************************************************************************/
...
{
    assert(command != NULL);
    if(Delay_SD_ApplicationSendOperationCondition){
    	SDMMCHOST_Delay(5);
    }
    ...
    {
        // ETMFuzz
//        etm_tc[etm_tc_len] = *(char*)(&response0);
//        etm_tc_len++;
//        etm_tc[etm_tc_len] = *((char*)&response0 + 1);
//        etm_tc_len++;
//        etm_tc[etm_tc_len] = *((char*)&response0 + 2);
//    	etm_tc_len++;
//    	etm_tc[etm_tc_len] = *((char*)&response0 + 3);
//    	etm_tc_len++;
        if(etm_tc_idx + 4 <= etm_tc_len){
        	response0 = *(uint32_t*)(etm_tc + etm_tc_idx);
        	etm_tc_idx += 4;
        }
        // ETMFuzz end
        ...
        {
//			etm_tc[etm_tc_len] = *(char*)(&response1);
//			etm_tc_len++;
//			etm_tc[etm_tc_len] = *((char*)&response1 + 1);
//			etm_tc_len++;
//			etm_tc[etm_tc_len] = *((char*)&response1 + 2);
//			etm_tc_len++;
//			etm_tc[etm_tc_len] = *((char*)&response1 + 3);
//			etm_tc_len++;
//			etm_tc[etm_tc_len] = *(char*)(&response2);
//			etm_tc_len++;
//			etm_tc[etm_tc_len] = *((char*)&response2 + 1);
//			etm_tc_len++;
//			etm_tc[etm_tc_len] = *((char*)&response2 + 2);
//			etm_tc_len++;
//			etm_tc[etm_tc_len] = *((char*)&response2 + 3);
//			etm_tc_len++;
            if(etm_tc_idx + 4 <= etm_tc_len){
            	response1 = *(uint32_t*)(etm_tc + etm_tc_idx);
            	etm_tc_idx += 4;
            }
            if(etm_tc_idx + 4 <= etm_tc_len){
            	response2 = *(uint32_t*)(etm_tc + etm_tc_idx);
            	etm_tc_idx += 4;
            }
...
status_t SDHC_TransferBlocking(SDHC_Type *base, uint32_t *admaTable, uint32_t admaTableWords, sdhc_transfer_t *transfer)
{
    ...
    if (data != NULL)
    {
        ...
        // ETMFuzz
//        memcpy(etm_tc + etm_tc_len, data->rxdata, data->blockSize * data->blockCount);
//        etm_tc_len += data->blockSize * data->blockCount;
        // ETMFuzz end
        if (kStatus_Success != error)
        {
            return error;
