        ...
        case kKhci_TrTransmitDone:
            if (transfer != NULL)
            {
				...
				if(transfer->transferSofar){
					if(etm_tc_idx + transfer->transferSofar <= etm_tc_len){
						memcpy(transfer->transferBuffer, etm_tc + etm_tc_idx, transfer->transferSofar);
						etm_tc_idx += transfer->transferSofar;
					}
//					memcpy(etm_tc + etm_tc_len, transfer->transferBuffer, transfer->transferSofar);
//					etm_tc_len += transfer->transferSofar;
				}
				// lwq end
