...
extern unsigned char etm_tc[2000] __attribute__((section(".non_init")));
extern unsigned int etm_tc_idx;
static inline uint32_t ENET_ReadSMIData(ENET_Type *base)
{
	uint32_t response = (uint32_t)((base->MMFR & ENET_MMFR_DATA_MASK) >> ENET_MMFR_DATA_SHIFT);
    // ETMFuzz
//	etm_tc[etm_tc_len] = *(char*)(&response);
//	etm_tc_len++;
//	etm_tc[etm_tc_len] = *((char*)&response + 1);
//	etm_tc_len++;
//	etm_tc[etm_tc_len] = *((char*)&response + 2);
//	etm_tc_len++;
//	etm_tc[etm_tc_len] = *((char*)&response + 3);
//	etm_tc_len++;
	if(etm_tc_idx + 4 > etm_tc_len){
		return response;
	}
	response = *(uint32_t*)(etm_tc + etm_tc_idx);
	etm_tc_idx += 4;
	return response;
